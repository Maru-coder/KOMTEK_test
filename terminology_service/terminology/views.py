...
class RefbookListView(APIView):
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter('date', openapi.IN_QUERY, description="Дата начала действия версии (формат ГГГГ-ММ-ДД)", type=openapi.TYPE_STRING)
        ],
...
class CheckElementView(APIView):
    @swagger_auto_schema(
        ...
        responses={200: openapi.Response(description="Результат валидации", schema=openapi.Schema(type=openapi.TYPE_OBJECT, properties={'valid': openapi.Schema(type=openapi.TYPE_BOOLEAN)}))}
    )
    def get(self, request, id):
